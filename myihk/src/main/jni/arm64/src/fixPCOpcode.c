...
int fixPCOpcodeArm(void *fixOpcodes , HK_INFO* pstInlineHook)
{
    uint64_t pc;
    uint64_t lr;
    int backUpPos = 0;
    ...
    pc = pstInlineHook->pBeHookAddr; //pc变量用于保存原本指令执行时的pc值
    lr = pstInlineHook->pBeHookAddr + pstInlineHook->backUpLength;
    ...
    if (!pstInlineHook->pHkFunAddr) {	//因为被hook函数直接调用是传参形式的，所以没有占用x0，就不需要恢复x0
...
int fixBcond(uint64_t pc, uint64_t lr, uint32_t instruction, uint32_t *trampoline_instructions, HK_INFO* pstInlineHook)
{
