...
int fixPCOpcodeArm(void *fixOpcodes , HK_INFO* info)
{
    ...
    currentOpcode = info->szbyBackupOpcodes + sizeof(uint8_t)*backUpPos;
    LOGI("sizeof(uint8_t) : %D", sizeof(uint8_t));

    pc = info->pBeHookAddr + 8; //pc变量用于保存原本指令执行时的pc值
    lr = info->pBeHookAddr + info->backUpLength;
    ...
    while(1) // 在这个循环中，每次都处理一个thumb命令
    {
        ...
        offset = fixPCOpcodeArm32(pc, lr, *currentOpcode, tmpFixOpcodes, info);
        ...
        if (isConditionBcode==1) { // the first code is B??
            if (backUpPos == 4) { // the second has just been processed
                ...
                tmpBcodeFix = pstInlineHook->pBeHookAddr + 8;
                ...
                tmpBcodeX = tmpBcodeX + 8 + pstInlineHook->pBeHookAddr;
            }
        }*/
        ...
        if (backUpPos < info->backUpLength)
        {
            ...
            LOGI("pstInlineHook->backUpLength : %d", info->backUpLength);
...
int fixPCOpcodeThumb(void *fixOpcodes , HK_INFO* info)
{
    ...
    pc = info->pBeHookAddr - 1 + 4; //pc变量用于保存原本指令执行时的pc值
    ...
    while(1) // 在这个循环中，每次都处理一个thumb命令
    {
        ...
        if(isThumb32(*currentOpcode)) //先判断它是thumb32还是thumb16
        {
            ...
            offset = fixPCOpcodeThumb32(pc, *currentThumb32high, *currentThumb32low, tmpFixOpcodes, info);
            LOGI("offset : %d",offset);
            memcpy(fixOpcodes+fixPos, tmpFixOpcodes, offset);
            ...
            offset = fixPCOpcodeThumb16(pc, *currentOpcode, tmpFixOpcodes, info);
            LOGI("offset : %d",offset);
            memcpy(fixOpcodes+fixPos, tmpFixOpcodes, offset);
...
int fixPCOpcodeThumb16(uint32_t pc, uint16_t instruction, uint16_t *trampoline_instructions, HK_INFO* info)
{
	int type;
	int offset;
    uint32_t new_entry_addr = (uint32_t)info->pNewEntryForOriFuncAddr;
    ...
    if (type == B1_BEQ_THUMB16 || type == B1_BNE_THUMB16 || type == B1_BCS_THUMB16 || type == B1_BCC_THUMB16 || type == B1_BMI_THUMB16
     ...
     || type == B1_BGE_THUMB16 || type == B1_BLT_THUMB16 || type == B1_BGT_THUMB16 || type == B1_BLE_THUMB16) {
        ...
        if(isTargetAddrInBackup(value-1,(uint32_t)info->pBeHookAddr,info->backUpLength)){
            //backup to backup !
            LOGI("BtoB in thumb16");
            int offset_in_backup = 0;
            int cnt = (value - 1 - ((uint32_t)info->pBeHookAddr - 1))/2;
            for(int i=0;i<cnt;i++){
                offset_in_backup += info->backUpFixLengthList[i];
            }
        ...
        if(isTargetAddrInBackup(value-1,(uint32_t)info->pBeHookAddr,info->backUpLength)){
            ...
            LOGI("CNT : %d",cnt);
            LOGI("VALUE : %x",value);
            LOGI("HOOK ADDR : %x",(uint32_t)info->pBeHookAddr)
            for(int i=0;i<cnt;i++){
                offset_in_backup += info->backUpFixLengthList[i];
...
/*        flag = (uint32_t)(instruction & 0xFFFFFF);
        ...
        value = x + pc;
        if(isTargetAddrInBackup(value, (uint32_t)info->pBeHookAddr, info->backUpLength)){
            LOGI("B TO B in Arm32");
            int offset_in_backup;
            int cnt = (value - (uint32_t)info->pBeHookAddr)/4;
            if(cnt==0){
                value = new_entry_addr;
            }else if(cnt==1){
                value = new_entry_addr + info->backUpFixLengthList[0];
            }else{
