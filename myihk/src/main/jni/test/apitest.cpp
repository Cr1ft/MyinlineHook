...
void hookArtInvoke(){
    ...
    if (artaddress != NULL) {
        invoke = getAddrbySym(artaddress,"_ZN3art9ArtMethod6InvokeEPNS_6ThreadEPjjPNS_6JValueEPKc");
        if(invoke != NULL){
...
#elif defined(__arm__)
    void *artaddress = dlopen("/system/lib/libart.so",RTLD_NOW);
    if (artaddress != NULL) {
        void *invoke = dlsym(artaddress,"_ZN3art9ArtMethod6InvokeEPNS_6ThreadEPjjPNS_6JValueEPKc");
        if(invoke != NULL){
            const RetInfo &rep = dump_replace(invoke, (void *) my_invoke, NULL, NULL, NULL);
            if (rep.status != success) {
                LE("hook dlopen error=%d", rep.status);
...
typedef void*(*def_my_fopen)(void* pathname,void* mode);
void* newopen(void* a,void* b){
    LE("%s",(char*)a)
    auto t = (def_my_fopen)getOriFunByHkFun((void*)(newopen));
    void* f = t(a,b);
    return f;
}
void hookopen(){
    void *handler = getModuleAddr("libc.so");
    void* oriopen =getAddrbySym(handler,"fopen");
    const RetInfo &rep = dump_replace(oriopen, (void*)newopen, NULL, NULL, NULL);
    if (rep.status != success) {
        LE("hook fopen error=%d", rep.status);
    }
...
extern "C"
JNIEXPORT void JNICALL
Java_com_zhuotong_myihk_MainActivity_hookopen(JNIEnv *env, jobject thiz) {
// TODO: implement hookArtInvoke()
void hookopen();
